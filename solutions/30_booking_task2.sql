WITH ClientBookings AS (
    -- Клиенты, которые забронировали больше 2 раз и останавливались более чем в 1 отеле
    SELECT
        c.ID_customer,
        c.name,
...
ClientSpentMoreThan500 AS (
    -- Клиенты, потратившие больше 500
    SELECT
        c.ID_customer,
        c.name,
        SUM(r.price) AS total_spent,
...
FROM
    ClientBookings cb
JOIN
    ClientSpentMoreThan500 csm ON cb.ID_customer = csm.ID_customer
ORDER BY
    cb.total_spent ASC;
