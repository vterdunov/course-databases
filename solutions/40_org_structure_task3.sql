WITH RECURSIVE ManagerHierarchy AS (
    -- Базовый случай: находим всех менеджеров, у которых есть подчиненные
    SELECT
        e.EmployeeID,
        e.Name,
        e.ManagerID,
    ...
    -- Рекурсивный случай: добавляем всех подчиненных каждого менеджера
    SELECT
        sub.EmployeeID,
        sub.Name,
        sub.ManagerID,
    ...
    FROM
        Employees sub
    INNER JOIN
        ManagerHierarchy mh ON sub.ManagerID = mh.EmployeeID
)

-- Получаем данные о менеджерах и их командах
SELECT
...
INNER JOIN
    Employees emp ON m.RootManagerID = emp.EmployeeID
LEFT JOIN
    Departments d ON emp.DepartmentID = d.DepartmentID
LEFT JOIN
