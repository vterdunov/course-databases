WITH HotelCategories AS (
    -- Определяем категорию отеля на основе средней стоимости номеров
    SELECT
        h.ID_hotel,
        CASE
...
CustomerPreferences AS (
    -- Определяем предпочтения клиентов по категориям отелей
    SELECT
        b.ID_customer,
        MAX(CASE
...
-- Выводим результат с именами клиентов
SELECT
    cp.ID_customer,
    c.name,
...
JOIN
    Customer c ON cp.ID_customer = c.ID_customer
ORDER BY
    CASE cp.preferred_hotel_type  -- сортируем по категории отеля
        WHEN 'Дешевый' THEN 1
        WHEN 'Средний' THEN 2
        WHEN 'Дорогой' THEN 3
    END;
